...
Статус: **разведка (этап 0)**. Всё ниже получено 2026-09-25 чтением GATT и пассивным прослушиванием уведомлений с PC (`tools/ble/recon.py`), без команд записи. Дорожка в режиме ожидания.
...
- [ ] Проверить управление через FTMS Control Point: Request Control → Start → Set Target Speed → Set Target Inclination → Stop. **Только с человеком у дорожки** (см. CLAUDE.md).
- [ ] Реальные значения полей при движении; поле с флагом бита 13 во втором пакете.
...
- [ ] Реакция дорожки на пропажу BLE во время движения (останавливается или продолжает).
