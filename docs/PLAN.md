...
- Видеть в реальном времени: время работы, скорость, наклон (% и градусы), дистанцию (общую и в разбивке по скорости/наклону).
...
| Xiaomi Redmi 6 4/64 (`cereus`, Helio P22) | Хаб, домашний мини-сервер | Будет с root, см. redmi6-homeserver |
...
- Профили: два пользователя, у каждого — лимит скорости, свои программы, своя история. Без паролей (только домашняя сеть).
...
| 2 | **Хаб MVP** | APK: подключение, телеметрия, REST/WS, простая страница с ± и СТОП, debug API, симулятор |
| 3 | **PWA и история** | Профили, тренировки, статистика, графики, установка PWA |
| 4 | **Программы** | Движок, встроенные P1–P12, редактор своих программ с повторами, профиль на графике, фиксация запуска с пульта дорожки |
| 5 | **Доработки** | Экспорт (CSV/GPX, возможно Strava), MCP-сервер, пульс (поручни или нагрудный датчик) |

Параллельно идёт подготовка телефона (разблокировка, root, ACC, Termux) — в redmi6-homeserver.

## 8. Открытые вопросы

- [ ] Версия MIUI/Android на Redmi 6.
- [ ] Есть ли у дорожки FTMS (`0x1826`) или только FitShow `FFF0`?
- [ ] Можно ли командой запустить встроенную программу? Сообщает ли дорожка номер программы и отрезка?
- [ ] Есть ли инструкция с таблицами программ P1–P12?
- [ ] Передаёт ли дорожка пульс с поручней по BLE?
